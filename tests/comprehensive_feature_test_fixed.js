...
const isSuccess = (response) => {
  return response.data.status === 1 || response.data.success === true;
};
...
async function testHealthCheck() {
  try {
    ...
    if (isSuccess(response) || response.data.message) {
...
async function testJobSeekerRegistration() {
  try {
    ...
    if (isSuccess(response) && response.data.data.token) {
    ...
    log(`✗ Job Seeker registration failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testEmployerRegistration() {
  try {
    ...
    log(`✗ Employer registration failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testLogin() {
  try {
    ...
    if (isSuccess(response) && response.data.data.token) {
      ...
      log(`  Token received: ${response.data.data.token.substring(0, 20)}...`, 'info');
      return true;
    }
    throw new Error('Login failed - no token received');
  } catch (error) {
    log(`✗ Login failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testGetAuthenticatedUser() {
  try {
    ...
    if (isSuccess(response) && response.data.data.user) {
      log('✓ Get authenticated user successful', 'success');
      log(`  Email: ${response.data.data.user.email}`, 'info');
      log(`  Role: ${response.data.data.user.role}`, 'info');
      return true;
    }
    throw new Error('Get user failed');
  } catch (error) {
    log(`✗ Get authenticated user failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobSeekerProfileManagement() {
  try {
    ...
    if (isSuccess(response)) {
      ...
      log(`  Full Name: ${profile.fullName}`, 'info');
      log(`  Skills: ${profile.skills?.join(', ')}`, 'info');
      return true;
    }
    throw new Error('Profile update failed');
  } catch (error) {
    log(`✗ Job Seeker profile management failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testEmployerProfileManagement() {
  try {
    ...
    if (isSuccess(response)) {
      ...
      const profile = response.data.data.profile || response.data.profile;
      log(`  Company: ${profile.companyName}`, 'info');
      log(`  Location: ${profile.location}`, 'info');
      return true;
    }
    throw new Error('Employer profile update failed');
  } catch (error) {
    log(`✗ Employer profile management failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobCreation() {
  try {
    ...
    if (isSuccess(response) && response.data.data.job) {
      createdJobId = response.data.data.job.id;
      log('✓ Job created successfully', 'success');
      log(`  Job ID: ${createdJobId}`, 'info');
      log(`  Title: ${response.data.data.job.title}`, 'info');
      return true;
    }
    throw new Error('Job creation failed - no job returned');
  } catch (error) {
    log(`✗ Job creation failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobListing() {
  try {
    ...
    if (isSuccess(response) && Array.isArray(jobs)) {
      log('✓ Job listing retrieved successfully', 'success');
      const pagination = response.data.data?.pagination || response.data.pagination;
      log(`  Total jobs: ${pagination?.total || jobs.length}`, 'info');
      if (jobs.length > 0) {
        log(`  First job: ${jobs[0].title}`, 'info');
      }
      return true;
    }
    throw new Error('Job listing failed - no jobs array returned');
  } catch (error) {
    log(`✗ Job listing failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobSearch() {
  try {
    ...
    const keywordJobs = response.data.data?.jobs || response.data.jobs || [];
    if (isSuccess(response)) {
      log(`✓ Keyword search successful (found ${keywordJobs.length} jobs)`, 'success');
    }
    
    // Test location filter
    log('Testing location filter...', 'info');
    response = await axios.get(`${API_URL}/jobs?location=San Francisco`);
    const locationJobs = response.data.data?.jobs || response.data.jobs || [];
    if (isSuccess(response)) {
      log(`✓ Location filter successful (found ${locationJobs.length} jobs)`, 'success');
    }
    
    // Test job type filter
    log('Testing job type filter...', 'info');
    response = await axios.get(`${API_URL}/jobs?jobType=full-time`);
    const typeJobs = response.data.data?.jobs || response.data.jobs || [];
    if (isSuccess(response)) {
      log(`✓ Job type filter successful (found ${typeJobs.length} jobs)`, 'success');
    }
    
    return true;
  } catch (error) {
    log(`✗ Job search failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobDetail() {
  try {
    ...
    if (isSuccess(response) && job) {
      log('✓ Job detail retrieved successfully', 'success');
      log(`  Title: ${job.title}`, 'info');
      log(`  Location: ${job.location}`, 'info');
      return true;
    }
    throw new Error('Job detail retrieval failed');
  } catch (error) {
    log(`✗ Job detail failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobUpdate() {
  try {
    ...
    if (isSuccess(response)) {
      ...
      log(`  Updated title: ${job.title}`, 'info');
      return true;
    }
    throw new Error('Job update failed');
  } catch (error) {
    log(`✗ Job update failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobApplication() {
  try {
    ...
    if (isSuccess(response) && application) {
      applicationId = application.id;
      log('✓ Job application submitted successfully', 'success');
      log(`  Application ID: ${applicationId}`, 'info');
      log(`  Status: ${application.status}`, 'info');
      return true;
    }
    throw new Error('Job application failed');
  } catch (error) {
    log(`✗ Job application failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testMyApplications() {
  try {
    ...
    if (isSuccess(response) && Array.isArray(applications)) {
      log('✓ Applications retrieved successfully', 'success');
      log(`  Total applications: ${applications.length}`, 'info');
      if (applications.length > 0) {
        log(`  First application status: ${applications[0].status}`, 'info');
      }
      return true;
    }
    throw new Error('Get applications failed');
  } catch (error) {
    log(`✗ Get my applications failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobApplicationsByEmployer() {
  try {
    ...
    log(`✗ Get job applications failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testApplicationStatusUpdate() {
  try {
    ...
    if (isSuccess(response)) {
      ...
      log(`  New status: ${application.status}`, 'info');
      return true;
    }
    throw new Error('Status update failed');
  } catch (error) {
    log(`✗ Application status update failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobSeekerDashboard() {
  try {
    ...
    const dashboard = response.data.data?.dashboard || response.data.dashboard;
    if (isSuccess(response) && dashboard) {
      log('✓ Job seeker dashboard retrieved successfully', 'success');
      log(`  Total applications: ${dashboard.totalApplications || 0}`, 'info');
      log(`  Profile completion: ${dashboard.profileCompletion || 0}%`, 'info');
      return true;
    }
    throw new Error('Dashboard retrieval failed');
  } catch (error) {
    log(`✗ Job seeker dashboard failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testEmployerDashboard() {
  try {
    ...
    if (isSuccess(response) && dashboard) {
      ...
      log(`  Total jobs: ${dashboard.totalJobs || 0}`, 'info');
      log(`  Total applications: ${dashboard.totalApplications || 0}`, 'info');
      return true;
    }
    throw new Error('Dashboard retrieval failed');
  } catch (error) {
    log(`✗ Employer dashboard failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testGetMyJobs() {
  try {
    ...
    log(`✗ Get my jobs failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testJobDeletion() {
  try {
    ...
    const jobToDelete = (createResponse.data.data?.job || createResponse.data.job).id;
    ...
    log(`✗ Job deletion failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
...
async function testWithdrawApplication() {
  try {
    ...
    const appToWithdraw = (createResponse.data.data?.application || createResponse.data.application).id;
    ...
    log(`✗ Application withdrawal failed: ${error.response?.data?.error?.message || error.message}`, 'error');
    return false;
  }
}
